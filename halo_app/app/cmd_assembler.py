...
class DtoAssemblerFactory(AbsBaseClass):
    ...
    @classmethod
    def get_assembler_by_request(cls, request: AbsHaloRequest) -> AbsCmdAssembler:
        if request.method_id in settings.CMD_ASSEMBLERS:
            cmd_assembler_type = settings.CMD_ASSEMBLERS[request.method_id]
            assembler: AbsCmdAssembler = Reflect.instantiate(cmd_assembler_type, AbsCmdAssembler)
            return assembler
        raise MissingCmdAssemblerException(request.method_id)
